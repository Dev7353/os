...
nextGroup ()
{
	...
	if (verbose == true)
		printf ("\t%s[NEXT GROUP IS %d]%s\n", ANSI_COLOR_GREEN,
			min_index, ANSI_COLOR_RESET);
	return min_index;

}

boolean
groupIsDone (int animal)
{
	boolean erg = true;
	for (int i = 0; i < prio.threads_per_group[animal]; ++i)
	{
...
void
calcGroupPriorities (int current_group)
{
...
initializeGlobals ()
{
	...
	for (int i = 0; i < GROUPS; ++i)
	{
		...
		waiting_times[i] =
			(double *) calloc (prio.threads_per_group[i] *
					   area.eating_times_per_group[i],
					   sizeof (double));
