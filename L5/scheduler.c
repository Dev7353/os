...
main (int argc, char *argv[])
{
	...
	while (1)
	{
		...
		c = getopt_long (argc, argv, "fh", long_options,
				 &option_index);
	...
	{
		fprintf (stderr,
			 "There must be at least one animal who wants to eat\n");
		return 1;
	}
	...
	{
		prio.priority[i] =
			(int *) calloc (prio.threads_per_group[i],
					sizeof (int));
	...
	assert (pthread_create
		(&schedule, NULL, (void *(*)(void *)) &scheduler, NULL) == 0);
...
eat (void *arg)
{
	...
	if (verbose == true)
		printf ("%s%d %s is done%s\n", ANSI_COLOR_YELLOW, param.id,
			param.animal_type, ANSI_COLOR_RESET);
}
	...
	while (true)
	{
		...
		for (int j = 0; j < prio.threads_per_group[animal]; ++j)
		{
			...
			while (true)
			{
				...
				else if (synchronize[animal][j] == false
					 && area.bowles == 1)
				{
