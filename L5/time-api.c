...
double
getMin (int animal, int animal_threads)
{
	...
	for (int i = 0;
	     i < animal_threads * area.eating_times_per_group[animal]; ++i)
	{
		if (waiting_times[animal][i] <= min
		    && waiting_times[animal][i] > 0)
...
double
getAvg (int animal, int animal_threads)
{
	if (area.eating_times_per_group[animal] == 0)
		return 0;
...
double
getMax (int animal, int animal_threads)
{
	if (area.eating_times_per_group[animal] == 0)
		return 0;
...
int
nextTimeSlot (int animal)
{
	int index = 0;

	for (int i = 0;
	     i <
	     prio.threads_per_group[animal] *
	     area.eating_times_per_group[animal]; ++i)
